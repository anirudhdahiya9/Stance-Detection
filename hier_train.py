...
from model import HierarchicalClassifier, HierarchicalRoberta
...
def evaluate(model, dataset, alpha, beta, batch_size, update_every, device):
    data_sampler = SequentialSampler(dataset)
    data_loader = DataLoader(dataset, batch_size, sampler=data_sampler)

    mmd_lossfn = nn.L1Loss(reduction='sum')
    nllloss = nn.NLLLoss()

    unrelated_index = torch.zeros(1).to(device)
    model.to(device)

    model.eval()

    tr_loss = 0.0
    total_n1 = torch.zeros(1)
    total_n2 = torch.zeros(1)
    total_n1_rep = torch.zeros(model.config.d_dim)
    total_n2_rep = torch.zeros(model.config.d_dim)

    for step, batch in enumerate(tqdm(data_loader)):
        batch = tuple(t.to(device) for t in batch)
        inputs = {"input_ids": batch[0], "attention_mask": batch[1]}
        batch_labels = batch[2]
        unrelated_prob, stance_prob, hidden_layer = model(**inputs)

        unrelated_labels = (batch_labels == unrelated_index).type(torch.long)
        batch_labels[unrelated_labels] = -100

        unrelated_loss = nllloss(unrelated_prob, unrelated_labels)
        stance_loss = alpha * nllloss(stance_prob, batch_labels)

        tr_loss += unrelated_loss.item()
        tr_loss += stance_loss.item()

        n1, n1_rep, n2, n2_rep = model.classifier.mmd_regularization(hidden_layer, labels=batch[2],
        ...
        if (step + 1) % update_every == 0:
            total_n1_rep /= total_n1
            total_n2_rep /= total_n2
            mmd_loss = -1 * beta * mmd_lossfn(total_n1_rep, total_n2_rep)

            tr_loss += mmd_loss.item()
...
def train(model, train_dataset, val_dataset, batch_size, update_every, num_epochs, tb_log_path, device, learning_rate,
          early_stopping_threshold, alpha, beta, out_path):
    ...
    # rel_score, report, val_loss, conf_matr = evaluate(model, val_dataset, alpha, beta, batch_size, update_every, device)
    ...
    for epoch in range(num_epochs):
        ...
        rel_acc, report, val_loss, conf_matr = evaluate(model, val_dataset, alpha, beta, batch_size, update_every, device)

        tb_logger.add_scalars('Running Losses', {'train_loss': tr_loss,
...
def main():
    parser = argparse.ArgumentParser(prog='Roberta factchecker', description='Hierarchical Roberta for '
                                                                             'factchecking.')
    ...
    parser.add_argument('--experiment_label', default='hier_model', type=str, help='Label for logs.')

    # Training args
    parser.add_argument('--cuda', type=bool, help="False if cpu mode needed.")
    parser.add_argument('--batch_size', default=6, type=int, help="Training batch size.")
    parser.add_argument('--update_every', default=10, type=int, help="Update params every n batches.")
    ...
    if args.mode == 'train':
        ...
        config.u_dim = 200
        config.d_dim = 100
        model = HierarchicalRoberta.from_pretrained(args.pretrained_model_path, config=config)
        ...
        model = HierarchicalRoberta.from_pretrained(model_path, config=config)
